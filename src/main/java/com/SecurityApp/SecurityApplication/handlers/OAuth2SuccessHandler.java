...
public class OAuth2SuccessHandler extends SimpleUrlAuthenticationSuccessHandler {
    ...
    public void onAuthenticationSuccess(HttpServletRequest request, HttpServletResponse response, Authentication authentication) throws IOException {
        ...
        String email=oAuth2User.getAttribute("email");//if we don't get any user with this email
        ...
        Cookie cookie=new Cookie("refreshToken",refreshToken);//creating cookie
        cookie.setHttpOnly(true);
        cookie.setSecure("production".equals(deployEnv));
        response.addCookie(cookie);


        String frontendUrl="http://localhost:8086/home.html?token="+accessToken;
