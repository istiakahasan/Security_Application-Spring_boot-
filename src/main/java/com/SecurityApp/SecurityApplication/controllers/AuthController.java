...
//    String token=authService.login(loginDto);//Step to login
        LoginResponseDto loginResponseDto=new LoginResponseDto();


        Cookie cookie=new Cookie("refreshToken",loginResponseDto.getRefreshToken());//creating cookie
    ...
    public ResponseEntity<LoginResponseDto> refresh(HttpServletRequest request){
        String refreshToken= Arrays.stream(request.getCookies())
                .filter(cookie -> "refreshToke".equals(cookie.getName())).findFirst()
                .map(cookie -> cookie.getValue())//(cookie::cookie)
